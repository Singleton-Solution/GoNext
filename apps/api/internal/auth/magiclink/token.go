...
// tokenBytes is the entropy width of one magic-link token. 32 bytes
// (256 bits) matches the password-reset token width — at this width
// brute force against a single token is infeasible for any plausible
// threat model. The hex encoding produces 64 characters, which fits
// in any URL query parameter without escaping.
const tokenBytes = 32
...
// occasionally copy a sign-in link from their email client into a
// browser by hand.
//
// crypto/rand is used directly because a predictable magic-link token
// is a one-click account-takeover primitive.
func generateToken() (string, error) {
	buf := make([]byte, tokenBytes)
	if _, err := rand.Read(buf); err != nil {
		return "", fmt.Errorf("magiclink: read entropy: %w", err)
	}
	return hex.EncodeToString(buf), nil
}

// hashToken returns the hex-encoded SHA-256 of the supplied plaintext
// token. The hex form is stored in magic_link_tokens.token_hash so
